...
router.get("/", async (req, res) => {
  try {
    ...
    if (req.query.gender) {
      ...
      query.gender = genderMap[req.query.gender.toLowerCase()] || req.query.gender;
    }
    ...
    if (req.query.category) {
      ...
      query.category = categoryMap[req.query.category.toLowerCase()] || req.query.category;
    }
    ...
    // ✅ brand → string (no $in unless you expect multiple)
    ...
    if (material && material.toLowerCase() !== "all") {
      const materialList = material.split(",").map(m => new RegExp(`^${m.trim()}$`, "i"));
      query.material = { $in: materialList };
    }
    ...
    // Sorting
   if (sortBy) {
  const sortKey = sortBy.toLowerCase(); // ✅ Convert once before comparing
