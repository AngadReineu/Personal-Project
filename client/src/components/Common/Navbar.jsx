        ...
        <>
            <nav className="Container mx-auto flex items-center justify-between py-4 px-6">
                ...
                <div className="flex items-center space-x-5">
                    <Link to="/admin" className="md:hidden block bg-red rounded text-white text-sm hover:text-black font-medium px-2 uppercase ">Admin</Link>
                    ...
                    <button onClick={toggleCartDrawer} className=" relative hover:text-black">
                        ...
                        {
                            cartItemCount > 0 && (
                                <span className="absolute -top-1 left-4 bg-[#ea2e0e] text-white text-xs rounded-full px-2 py-0.5">
                                    {cartItemCount}
                                </span>
                            )
                        }
            ...
            <Cart draw={draw} setDraw={setDraw} toggleCartDrawer={toggleCartDrawer} />

            {/* Mobile Navigation */}

            <div className={`fixed top-0 left-0 w-3/4 sm:w-1/2 md:w-1/3 h-full
         bg-white shadow-lg transform transition-transform duration-300 z-50 ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}>
